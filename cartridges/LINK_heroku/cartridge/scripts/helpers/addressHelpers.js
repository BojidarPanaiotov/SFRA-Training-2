...
function saveAddress(address, customer, addressId) {
    ...
    if (HookMgr.hasHook('custom.customer.data')) {
        var result = HookMgr.callHook('custom.customer.data',
            'saveUserAddress', 
            customer.profile == null? customer.raw.profile.customerNo:customer.profile.customerNo,
            addressId,
            address.firstName,
            address.lastName,
            address.address1,
            ...
            address.states.stateCode,
            address.phone);

        if(result){
            saveToDb(address, customer, addressId);
        }
    }else{
        saveToDb(address, customer, addressId);
    }
...
function saveToDb(address, customer, addressId) {
